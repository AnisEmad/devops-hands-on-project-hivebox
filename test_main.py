...
def make_sensor_data(value: str, minutes_ago: int = 0, title: str = "Temperature") -> dict:
...
class TestTemperatureEndpoint:
    ...
    def test_sensor_title_case_insensitive(self, client):
        """Sensor matching is case-insensitive ('TEMPERATURE', 'Temperatur', …)."""
        data = [
    ...
    def test_excludes_measurements_older_than_one_hour(self, client):
        ...
        with patch("main.get_box_data", new=AsyncMock(side_effect=data)):
            response = client.get("/temperature")
        # All stale → error returned as a list: ["error: no temperature found"]
        body = response.json()
    ...
    def test_excludes_measurement_exactly_at_one_hour_boundary(self, client):
        """The check is `<= timedelta(hours=1)`, so a reading at exactly 60 minutes
        is on the boundary and gets excluded (elapsed == limit, not elapsed < limit).
        This documents the actual behaviour of the production code."""
        data = [
    ...
    def test_returns_error_when_no_temperature_sensor_found(self, client):
        """Boxes with no temperature sensor should be skipped; error returned if none found."""
        no_temp = {
            "sensors": [
                {"title": "Humidity", "lastMeasurement": {"value": "60", "createdAt": "2099-01-01T00:00:00Z"}},
            ]
        }
